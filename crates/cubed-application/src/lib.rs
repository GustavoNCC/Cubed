...
mod tests {
    ...
    impl FileSystemManager for NoopFs {
        ...
        async fn init_server_dirs(&self, _: &str, _: &str) -> ApplicationResult<()> {
            Ok(())
        }
        async fn delete_server_dir(&self, _: &str, _: &str) -> ApplicationResult<()> {
            Ok(())
        }
        fn server_dir(&self, servers_dir: &str, name: &str) -> String {
            format!("{}/{}", servers_dir, name)
        }
        async fn ensure_writable(&self, _: &str) -> ApplicationResult<()> {
            Ok(())
        }
    }

    fn make_fs() -> Arc<NoopFs> {
        Arc::new(NoopFs)
    }
